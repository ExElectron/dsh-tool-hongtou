...
// 公章解析（配置驱动，默认跳过）：
//   seal 未设置或 off/false → 仅当工作区存在 seal.png（用户自放）时盖章，否则跳过（干净版面）
//   seal = default / true / auto → 内置默认章 assets/seal-default.png
//   seal = legacy               → 内置前一款章 assets/seal-legacy.png（可选）
//   seal = 路径                  → 使用指定图片（相对路径按工作区解析）
...
async function resolveSeal(workspace, config = {}) {
  ...
  const userFile = await firstExisting([join(workspace, "seal.png")]);
  if (setting === true || setting === "default" || setting === "auto") {
    return userFile ?? join(PLUGIN_DIR, "assets", "seal-default.png");
  }
  if (setting === "legacy") {
    return userFile ?? join(PLUGIN_DIR, "assets", "seal-legacy.png");
  }
